...
public class CocoaDM extends CongestionControlLayer {
...
//    private static float last = -1;
    ...
    public CocoaDM(String tag, Configuration config, boolean strong) {
        super(tag, config);
        this.strong=strong;
        setDithering(true);
    }
    ...
    private static class CocoaRemoteEndpoint extends RemoteEndpoint {
        ...
        public synchronized void processRttMeasurement(RtoType rtoType, long measuredRTT) {
            ...
            if (RTTq.size()==3) {
                long RTT0 = RTTq.remove();
                long RTT1 = RTTq.remove();
                long RTT2 = RTTq.remove();
                float CCr = RTT0/RTT1;
                float PCr = RTT1/RTT2;
                float Thm = 100;
                float Thd =100;
